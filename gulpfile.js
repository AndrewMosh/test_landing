...
function images() {
    return src(path.src.img).pipe(src(path.src.img)).pipe(dest(path.build.img)).pipe(browsersync.stream());
}
...
let build = gulp.series(clean, html, gulp.parallel(css, video, js, images, fonts));
